...
class CottageInformationInput(MDTextField):
...
class CottageInModifier(BoxLayout):
    ...
    def remove(self):
        self.parent.parent.parent.remove_cottage_by_itself(self , self.cottage_id)

    def create_receipt(self):
        backend.createReciept(self.persons , self.items , self.electric , self.water , self.price , self.total)
...
class CottageSelectedContainer(BoxLayout) :
    ...
    def remove_cottage_by_itself(self , cottage : CottageInModifier  , cottage_id : int ):
        self.cottage_selected_container.remove_widget(cottage)
        self.parent.selectingCottage(cottage_id , False)
...
class CottageSelectedModifier(BoxLayout) :
    ...
    def selectingCottage(self , cottage_id : int , value : bool):
        ...
        self.parent.cottage_selections.selectingCottage( cottage_id , value )
...
class CottageSelections(BoxLayout):
    ...
    def displayAll(self , find = None):
        if self.current == 'all' and not find :
            return
    ...
    def displaySelected(self , find = None):
        if self.current == 'selected' and not find :
            return
        self.cottage_container.list_of_cottage.clear_widgets()
        widgets = []
        for data in self.parent.datas.get_all_data():
    ...
    def displayUnselected(self , find = None):
        if self.current == 'unselected' and not find :
            return
        self.cottage_container.list_of_cottage.clear_widgets()
        widgets = []
        for data in self.parent.datas.get_all_data():
    ...
    def findCottage(self , interval):
        if self.past_finding == self.finding :
            return
        self.past_finding = self.finding
        print(self.finding)
        if self.current == "all" :
            self.displayAll(self.finding)
            print("all")
        elif self.current == 'selected' :
            self.displaySelected(self.finding)
            print("selected")
        else:
            self.displayUnselected(self.finding)
            print("unselected")

    # ---> Built in method
    def on_kv_post(self, base_widget):
        Clock.schedule_interval(self.findCottage , 1 / 30)
...
class CottageApp(MDApp):
    ...
    def build(self):
        self.icon = "Cottage Pictures/big cottage, 500.webp"
